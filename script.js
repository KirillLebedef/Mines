    ...
    faqButton.addEventListener("click", () => {
        if (typeof Telegram !== 'undefined' && Telegram.WebApp) {
            Telegram.WebApp.showPopup({
                title: "Правила работы GunWIN_BOT",
                message:
                    "1. Суть работы:\n" +
                    "   - GunWIN_BOT анализирует ваш Hash с сайта 1win и определяет безопасные клетки на игровом поле.\n" +
                    "   - Все сигналы точны и соответствуют текущему раунду.\n\n" +
                    "2. Выбор мин:\n" +
                    "   - Укажите количество мин на игровом поле с помощью кнопок - и +.\n" +
                    "   - Доступные варианты: 1, 3, 5 или 7 мин.\n\n" +
                    "3. Как использовать:\n" +
                    "   - Зайдите на сайт 1win и начните игру.\n" +
                    "   - Сделайте ставку, чтобы бот получил Hash.\n" +
                    "   - В GunWIN_BOT выберите количество мин и нажмите \"GET SIGNAL\".\n" +
                    "   - Следуйте сигналам системы и нажимайте только на безопасные клетки на игровом поле.\n\n" +
                    "4. Важные моменты:\n" +
                    "   - Нажимайте \"GET SIGNAL\" только после того, как поставили на сайте 1win.\n" +
                    "   - Если вы начали новый раунд, обновите количество мин в боте перед получением сигналов.\n\n" +
                    "Удачи!",
                buttons: [
            ...
            alert("Telegram WebApp не поддерживается. Убедитесь, что вы открыли приложение через Telegram.");
        }
    });
});
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const loadingText = document.querySelector(".loading-text"); // Для текста под полосой
    ...
    // Массив сообщений для текста под полосой
    const messages = [
        "Подключаемся к вашему HASH...",
        "Загружаем ресурсы...",
    ...
    const textInterval = setInterval(() => {
        ...
        index = (index + 1) % messages.length; // Зацикливаем индексы
    }, 1500);
    ...
    // Скрываем экран загрузки через 5 секунд
    setTimeout(() => {
        clearInterval(textInterval); // Останавливаем смену текста
        loadingScreen.style.display = "none"; // Скрываем экран загрузки
        container.style.display = "flex"; // Показываем основной контент
    }, 5000);
