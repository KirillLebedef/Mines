...
// Функция для вызова вибрации через Telegram API
function triggerVibration(type = "impact", style = "light") {
    if (typeof Telegram !== 'undefined' && Telegram.WebApp && Telegram.WebApp.HapticFeedback) {
        Telegram.WebApp.HapticFeedback[type]({ style });
    }
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Добавляем вибрацию при нажатии на плюс
    plusBtn.addEventListener("click", () => {
        if (count < 7) {
            count = count === 1 ? 3 : count === 3 ? 5 : 7;
            updateCounter();
            triggerVibration("impact", "light"); // Легкая вибрация
        } else {
            triggerVibration("notification", "error"); // Ошибочная вибрация
        }
    });

    // Добавляем вибрацию при нажатии на минус
    minusBtn.addEventListener("click", () => {
        if (count > 1) {
            count = count === 7 ? 5 : count === 5 ? 3 : 1;
            updateCounter();
            triggerVibration("impact", "medium"); // Средняя вибрация
        } else {
            triggerVibration("notification", "warning"); // Предупреждающая вибрация
        }
    });

    // Добавляем вибрацию при нажатии на кнопку GET SIGNAL
    getSignalBtn.addEventListener("click", () => {
        if (isCooldown) {
            triggerVibration("notification", "error"); // Ошибка при кулдауне
            return;
        }

        // Логика генерации сигнала...
        triggerVibration("impact", "heavy"); // Сильная вибрация
    });
    ...
    plusBtn.addEventListener("click", () => {
        if (count < 7) {
            count = count === 1 ? 3 : count === 3 ? 5 : 7;
            updateCounter();
            vibratePattern([100, 50, 100]); // Вибрация при нажатии на плюс
        } else {
            vibratePattern([200, 100, 200]); // Вибрация при превышении лимита
        }
    });
    ...
    minusBtn.addEventListener("click", () => {
        if (count > 1) {
            count = count === 7 ? 5 : count === 5 ? 3 : 1;
            updateCounter();
            vibratePattern([100]); // Вибрация при нажатии на минус
        } else {
            vibratePattern([200]); // Вибрация при достижении минимума
        }
    });
    ...
    getSignalBtn.addEventListener("click", () => {
        if (isCooldown) {
            ...
            vibratePattern([100]); // Вибрация при попытке нажать во время кулдауна
            return;
        }
            ...
            if (!selectedCells.includes(index)) {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        vibratePattern([100]); // Вибрация при появлении звезды
                    }, 0);
        ...
        isCooldown = true;
        vibratePattern([100]); // Вибрация при успешном нажатии кнопки
